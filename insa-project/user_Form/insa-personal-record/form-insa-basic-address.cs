...
namespace insa_project
{
    ...
    public partial class form_insa_basic_address : Form
    {
        insa_basic insa_basic;

        public form_insa_basic_address(insa_basic a)
        {
            InitializeComponent();
            insa_basic = a;
        }

        private void form_insa_basic_address_Load(object sender, EventArgs e)
        {
        ...
        {
            insa_basic.address_box.Text = dataGridView1.Rows[e.RowIndex].Cells["도로명주소"].Value.ToString();
            insa_basic.zip_box.Text = dataGridView1.Rows[e.RowIndex].Cells["우편번호"].Value.ToString();
            //dataGridView1.Rows[e.RowIndex].Cells["도로명주소"].Value.ToString().Split(' ')[0]);
            this.Close();
        }
